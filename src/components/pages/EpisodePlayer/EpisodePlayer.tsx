...
export default function EpisodePlayer() {
  const { season: seasonParam, episode: episodeParam } = useParams<{
    season: string;
    episode: string;
  }>();
  ...
  const currentIndex = episode
    ? episodes.findIndex((ep) => ep.code === episode.code)
    : -1;
  ...
  return (
    <div className={styles.container}>
      ...
      <nav className={styles.navigation}>
        <button
          onClick={() =>
            previousEpisode &&
            navigate(
              `/episode/${previousEpisode.season}/${previousEpisode.episode}`
            )
          }
          ...
          {previousEpisode ? (
            <>
              ...
              <span className={styles.navInfo}>
                {String(previousEpisode.season).padStart(2, '0')}x
                {String(previousEpisode.episode).padStart(2, '0')} -{' '}
                <span className={styles.navEpisodeName}>
        ...
        <button
          onClick={() =>
            nextEpisode &&
            navigate(`/episode/${nextEpisode.season}/${nextEpisode.episode}`)
          }
        ...
        >
          {nextEpisode ? (
            <>
              ...
              <span className={styles.navInfo}>
                {String(nextEpisode.season).padStart(2, '0')}x
      ...
      <div className={styles.detailsCard}>
        <h3>Detalles</h3>
        <p>
          <strong>Código:</strong> {episode.code}
        </p>
        <p>
        ...
        <p>
          <strong>Episodio Absoluto:</strong> #{episode.absoluteEpisode}
        </p>
        <p>
        ...
        <p>
          <strong>Tipo:</strong>{' '}
          <span
            className={`${styles.detailBadge} ${episode.isCanon ? styles.canonBadge : styles.fillerBadge}`}
          >
        ...
        <p>
          ...
          {watched ? (
            ...
            <span style={{ color: 'var(--text-tertiary)' }}>
              Se marcará como visto en 10 segundos...
            </span>
          )}
        </p>
      </div>
